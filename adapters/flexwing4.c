 ...
 *   pk = concat(pk_M, pk_X)
 */
static int flexwing4_keypair(uint8_t *pk, uint8_t *sk)
{
 ...
 *   ct = concat(ct_M, ct_X)
 */
static int flexwing4_encaps(uint8_t *ct, uint8_t *ss, const uint8_t *pk)
{
    ...
    ret = flexwing4_combiner(ss, ss_m, ss_x, ct_x, pk_x);
 ...
 */
static int flexwing4_decaps(uint8_t *ss, const uint8_t *ct, const uint8_t *sk)
{
...
const crypto_kem_ops_t flexwing4_ops = {
    .type    = ALG_FLEXWING4,
    .name    = "Flexwing4 (ML-KEM-512 + X25519 + Sparkle-Hash384)",
    .keygen  = flexwing4_keypair,
    .encaps  = flexwing4_encaps,
    .decaps  = flexwing4_decaps,
};