...
{
    if (platform_rng_generate(key,   AES_GCM_KEY_BYTES)   != CRYPTO_SUCCESS)
        return CRYPTO_ERROR;
    if (platform_rng_generate(nonce, AES_GCM_NONCE_BYTES) != CRYPTO_SUCCESS)
        return CRYPTO_ERROR;
