 ...
 */
static int flexwing1_combiner(uint8_t ss[FLEXWING_SS_BYTES],
                              ...
                              const uint8_t pk_x[X25519_PK_BYTES])
{
    ...
    int ret = lwc_xoodyak_hash_ops.hash(ss, FLEXWING_SS_BYTES, buf, off);
 ...
 *   pk = concat(pk_M, pk_X)
 */
static int flexwing1_keypair(uint8_t *pk, uint8_t *sk)
{
 ...
 *   ct = concat(ct_M, ct_X)
 */
static int flexwing1_encaps(uint8_t *ct, uint8_t *ss, const uint8_t *pk)
{
 ...
 */
static int flexwing1_decaps(uint8_t *ss, const uint8_t *ct, const uint8_t *sk)
{
...
const crypto_kem_ops_t flexwing1_ops = {
    .type    = ALG_FLEXWING1,
    .name    = "FlexWing1 (ML-KEM-512 + X25519 + Xoodyak-Hash)",
    .keygen  = flexwing1_keypair,
    .encaps  = flexwing1_encaps,
    .decaps  = flexwing1_decaps,
};