 ...
 *   - lwc_sparkle_hashxof_ops (Xoodyak-Hash, used as the combiner hash)
 *
 ...
 */
static int flexwing3_combiner(uint8_t ss[FLEXWING_SS_BYTES],
                              ...
                              const uint8_t pk_x[X25519_PK_BYTES])
{
    ...
    int ret = lwc_sparkle_hashxof_ops.hash(ss, FLEXWING_SS_BYTES, buf, off);
 ...
 *   pk = concat(pk_M, pk_X)
 */
static int flexwing3_keypair(uint8_t *pk, uint8_t *sk)
{
 ...
 *   ct = concat(ct_M, ct_X)
 */
static int flexwing3_encaps(uint8_t *ct, uint8_t *ss, const uint8_t *pk)
{
    ...
    ret = flexwing3_combiner(ss, ss_m, ss_x, ct_x, pk_x);
 ...
 */
static int flexwing3_decaps(uint8_t *ss, const uint8_t *ct, const uint8_t *sk)
{
...
const crypto_kem_ops_t flexwing3_ops = {
    .type    = ALG_FLEXWING3,
    .name    = "FlexWing3 (ML-KEM-512 + X25519 + Sparkle-Hash)",
    .keygen  = flexwing3_keypair,
    .encaps  = flexwing3_encaps,
    .decaps  = flexwing3_decaps,
};